...
pub fn report_error(source: &[u8], error: &(dyn miette::Diagnostic + Send + Sync + 'static)) {
    eprintln!("Error: {:?}", ErrorReporter(error, source))
}

impl std::fmt::Debug for ErrorReporter<'_> {
...
impl<'src> miette::Diagnostic for ErrorReporter<'src> {
